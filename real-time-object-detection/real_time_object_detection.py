...
IGNORE = set(["person","bird","bus","car","cow","dog","horse","motorbike",
	"sheep"])
...
while True:
	...
	for i in np.arange(0, detections.shape[2]):
		...
		if confidence > args["confidence"]:
			...
			if CLASSES[idx] not in IGNORE:
				continue
			box = detections[0, 0, i, 3:7] * np.array([w, h, w, h])
			(startX, startY, endX, endY) = box.astype("int")
			#(startX+endX)/2,(endX+endY)/2 = center
			X=print("-coordinates",(startX,startY,endX,endY))
			...
			x=(grayFrame[centerX,centerY])
			#print(depth_image.shape)
			#x=(grayFrame[508,718])
			dist=(0.02591*x)+0.03047
			dist=round(dist,2)
			print("The distance of the object from camera is",dist,"meters")
